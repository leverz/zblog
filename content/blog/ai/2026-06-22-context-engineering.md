...
我读到这个的时候立刻联想到自己的系统：如果鸢-信息流一次要处理 20 条链接，第 3 条链接的信息到了第 20 步还能被准确召回吗？根据这个原理，答案很可能是**不能**。
...
**关键洞察：不是某一项太大，是 7 项同时竞争。**

我在自己的系统里对号入座：一次晨报生成任务，System Prompt、Tool Definitions、Tool Call Results（20 条链接摘要）、RAG、Memory、Agent State 会同时存在。其中**读取摘要最可能先成为瓶颈**——每处理一条链接就新增 5K-10K tokens，增长最快、最不可控。
...
**原理**：LLM 处理 token 时会计算 key-value 表示（昂贵），推理提供商**缓存**这些表示。如果两次 API 调用的**前缀相同** → 复用缓存，只处理新加的 token。
